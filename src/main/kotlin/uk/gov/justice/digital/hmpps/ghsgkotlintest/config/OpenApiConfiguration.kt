package uk.gov.justice.digital.hmpps.ghsgkotlintest.config
...
class OpenApiConfiguration(buildProperties: BuildProperties) {
  ...
  fun customOpenAPI(): OpenAPI = OpenAPI()
    .servers(
      listOf(
        Server().url("https://gh-sg-kotlin-test-dev.hmpps.service.justice.gov.uk").description("Development"),
        Server().url("https://gh-sg-kotlin-test-preprod.hmpps.service.justice.gov.uk").description("Pre-Production"),
        Server().url("https://gh-sg-kotlin-test.hmpps.service.justice.gov.uk").description("Production"),
    ...
    .info(
      Info().title("Gh Sg Kotlin Test").version(version)
    ...
    .components(
      Components().addSecuritySchemes(
        "gh-sg-kotlin-test-ui-role",
        SecurityScheme().addBearerJwtRequirement("ROLE_TEMPLATE_KOTLIN__UI"),
      ),
    )
    .addSecurityItem(SecurityRequirement().addList("gh-sg-kotlin-test-ui-role", listOf("read")))
}
